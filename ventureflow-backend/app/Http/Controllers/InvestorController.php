...
class InvestorController extends Controller
{
    ...
    {
        $buyers = Investor::where(function($q) {
    ...
    public function getLastSequence(Request $request)
    {
        ...
        try {
            $lastBuyer = Investor::where('buyer_id', 'LIKE', $prefix . '%')
                ->select('buyer_id')
                ->get()
    ...
    public function companyOverviewStore(Request $request)
    {
        try {
            ...
            if (!$investor || ($data['buyer_id'] && $data['buyer_id'] !== $investor->buyer_id)) {
                $code = $data['buyer_id'];
                $exists = Investor::where('buyer_id', $code)
                    ->when($investor, function($q) use ($investor) { $q->where('id', '!=', $investor->id); })
                    ->exists();
                if ($exists) {
            ...
            if ($investor && $investor->company_overview_id) {
                // Update existing company overview
                $overview = InvestorsCompanyOverview::find($investor->company_overview_id) ?? new InvestorsCompanyOverview();
            } else {
                // Create new company overview
                $overview = new InvestorsCompanyOverview();
                $isNewOverview = true;
            }
            ...
            // Link to Buyer
            if ($investor) {
                $investor->company_overview_id = $overview->id;
                
                // Only update buyer_id if a valid value is provided (not null/empty)
                if (!empty($data['buyer_id'])) {
                    $investor->buyer_id = $data['buyer_id'];
                }

                if (isset($data['profile_picture'])) {
                    $investor->image = $data['profile_picture'];
                }
                ...
                ActivityLog::create([
                    'user_id' => \Auth::id(),
                    'loggable_id' => $investor->id,
                    'loggable_type' => Investor::class,
                    'type' => 'system',
                    'content' => "Investor profile updated: " . ($overview->reg_name ?? ''),
                ]);

                // MatchIQ: Recompute matches after investor profile update
                ComputeMatchesJob::dispatch('buyer', $investor->id)->delay(5);
            } else {
                // Create new Investor if it doesn't exist
                ...
                ActivityLog::create([
                 ...
                 try {
                    ...
                    Notification::send($admins, new NewRegistrationNotification('Buyer', $overview->reg_name ?? 'new Investor', $investor->id, Auth::user()));
                } catch (\Exception $e) {
    ...
    {
        try {
            $investor = Investor::find($request->input('buyer_id'));
            ...
            if ($investor->target_preference_id) {
                $preference = InvestorsTargetPreferences::find($investor->target_preference_id);

                if (!$preference) {
                    // If the record somehow doesn't exist, fallback to a new instance
                    $preference = new InvestorsTargetPreferences();
                }
            } else {
                $preference = new InvestorsTargetPreferences();
            }
            ...
            if (!$investor->target_preference_id) {
                $investor->target_preference_id = $preference->id;
                $investor->save();
            }

            return response()->json([
                'message' => $investor->wasRecentlyCreated ? 'Target preferences created successfully.' : 'Target preferences updated successfully.',
                'data' => $investor->id,
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function financialDetailsStore(Request $request)
    {
        try {
            ...
            $investor = Investor::find($buyerId);
            ...
            if ($investor->financial_detail_id) {
                $financialDetail = InvestorsFinancialDetails::find($investor->financial_detail_id);
                if (!$financialDetail) {
                    $financialDetail = new InvestorsFinancialDetails();
                }
            } else {
                $financialDetail = new InvestorsFinancialDetails();
            }
            ...
            if (!$investor->financial_detail_id) {
                $investor->financial_detail_id = $financialDetail->id;
                $investor->save();
            }
    ...
    public function partnershipDetailsStore(Request $request)
    {
        try {
            ...
            $investor->partnership_detail_id = $partnership->id;
            $investor->save();
    ...
    public function teaserCenterStore(Request $request)
    {
        try {
            ...
            $teaserCenter = $investor->teaser_center_id
                ? InvestorsTeaserCenter::find($investor->teaser_center_id) ?? new InvestorsTeaserCenter()
                : new InvestorsTeaserCenter();
            ...
            if (!$investor->teaser_center_id) {
                $investor->teaser_center_id = $teaserCenter->id;
                $investor->save();
            }
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            DB::transaction(function () use ($idsToDelete, &$deletedCount) {
                ...
                $buyers = Investor::whereIn('id', $idsToDelete)->get();
                ...
                ActivityLog::where('loggable_type', Investor::class)
                ...
                $deletedCount = Investor::whereIn('id', $idsToDelete)->delete();

                // 5. Clean up the detailed records (orphaned after buyer is deleted)
                if (!empty($overviewIds)) {
                    InvestorsCompanyOverview::whereIn('id', $overviewIds)->delete();
                }
                if (!empty($preferenceIds)) {
                    InvestorsTargetPreferences::whereIn('id', $preferenceIds)->delete();
                }
                if (!empty($financialIds)) {
                    InvestorsFinancialDetails::whereIn('id', $financialIds)->delete();
                }
                if (!empty($partnershipIds)) {
                    BuyersPartnershipDetails::whereIn('id', $partnershipIds)->delete();
                }
                if (!empty($teaserIds)) {
                    InvestorsTeaserCenter::whereIn('id', $teaserIds)->delete();
                }
            });
