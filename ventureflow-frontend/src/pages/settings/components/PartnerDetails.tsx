...
import { Mail, Globe, Building2, User, KeyRound, PowerOff, Power, Loader, Camera } from 'lucide-react';
...
const PartnerDetails: React.FC = () => {
    ...
    const handleToggleActive = async () => {
        if (!partner?.user?.id) return;
        const currentlyActive = partner.user.is_active !== false;
        setStatusSaving(true);
        try {
            await api.patch(`/api/admin/users/${partner.user.id}/status`, {
                                ...
                                <div className="flex-1 max-w-[403px] p-3 bg-[rgba(249,250,251,0.5)] border border-[#F3F4F6] rounded">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-base font-medium text-gray-900">{partner.user?.email || 'Not set'}</span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div className="flex flex-col gap-4">
                                            <div className="flex items-start justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-base font-medium text-gray-900">{overview.contact_person_name}</span>
                                                ...
                                                <div className="pt-4 border-t border-[#F3F4F6] flex flex-col gap-3">
                                                    {overview.contact_person_email && (
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-xs font-normal text-gray-600">{overview.contact_person_email}</span>
                                                        </div>
                                                    )}
                    ...
                    <div className="w-[287px] shrink-0 space-y-10">
                        ...
                        {currentRole === 'System Admin' && partner.user?.id && (
                            <div className="space-y-3 pt-2 border-t border-gray-100">
                                ...
                                {/* Activate / Deactivate */}
                                {(() => {
                                    const isActive = partner.user!.is_active !== false;
                                    return (
                                        <button
