
import React from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft } from 'lucide-react';
import { InvestorRegistration } from './InvestorRegistration';

const AddBuyer: React.FC = () => {
    const navigate = useNavigate();

    return (
        <div className="flex flex-col h-full min-h-screen bg-gray-50 font-poppins">
            {/* Header */}
            <div className="flex items-center gap-4 px-4 md:px-6 py-4 bg-white border-b">
                <button
                    type="button"
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#064771] hover:bg-[#053a5c] text-white text-sm font-medium transition-all shadow-sm active:scale-95"
                    onClick={() => navigate('/prospects?tab=investors')}
                >
                    <ArrowLeft className="w-4 h-4" />
                    <span>Back</span>
                </button>
                <h1 className="text-xl md:text-2xl font-semibold text-gray-900">Add Investor</h1>
            </div>

            {/* Main Content */}
            <div className="flex-1 overflow-auto p-4 md:p-6">
                <div className="max-w-7xl mx-auto">
                    <InvestorRegistration />
                </div>
            </div>
        </div>
    );
};

export default AddBuyer;
