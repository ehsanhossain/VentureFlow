...
const DriveFilePreview: React.FC<DriveFilePreviewProps> = ({ file, previewUrl, onClose, onDownload }) => {
            ...
            <div className={`relative bg-white rounded-lg shadow-2xl flex flex-col z-[101] transition-all duration-200 ${fullscreen ? 'w-full h-full m-0 rounded-none' : 'w-full max-w-4xl h-[85vh]'}`}>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
                    <div className="flex items-center gap-2.5 min-w-0">
                        <img src={getFileIconSrc(file.original_name)} alt="" className="w-7 h-7 shrink-0" draggable={false} />
                        <div className="min-w-0">
                            <p className="text-sm font-medium text-gray-800 truncate">{file.original_name}</p>
                            <p className="text-[11px] text-gray-400">{formatFileSize(file.size)} · v{file.current_version}</p>
                        </div>
                    </div>
